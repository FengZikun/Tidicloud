...
export default{
	...
	methods:{
		...
		init(){
			...
			var success=function(res){
				...
				if(res.tradeMarkImgUrl!==null){
					self.imgSrcA=`url('${"http://cloud.tidicloud.com"+res.tradeMarkImgUrl}')`;
				}
				if(res.tradeMarkLicense!==null){
					self.imgSrcB=`url('${"http://cloud.tidicloud.com"+res.tradeMarkLicense}')`;
				}
				if(res.industrialProductionLicense!==null){
					self.imgSrcC=`url('${"http://cloud.tidicloud.com"+res.industrialProductionLicense}')`;
				}
				if(res.foodProductionLicence!==null){
					self.imgSrcD=`url('${"http://cloud.tidicloud.com"+res.foodProductionLicence}')`;
				}
				if(res.bankAccountOpeningLicense!==null){
					self.imgSrcE=`url('${"http://cloud.tidicloud.com"+res.bankAccountOpeningLicense}')`;
				}
			}
		...
		commit(){
			...
			$.ajax({
				url:'http://cloud.tidicloud.com/cloud_code/POST/user/uploadRegistrationPic.do',
				type:'post',
				dataType:'json',
				data:picData,
				cache: false,
