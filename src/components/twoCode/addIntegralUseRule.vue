			...
			<div class="choosepro" >
				...
				<ul class="choosepro-main" v-show='gift' :class='{gift:gift}'>
					...
					<li class="pro-li" v-for='item in gifts'>
						<span class="pro-li-span first" v-bind:data-id='item.id'><span class="check-box checkshu" @click='selectThis' style="margin-left:10%;margin-right:5%;"></span><span style="display: inline-block;vertical-align: middle;overflow: hidden"><img style="width:45px;margin-left:20px;" v-bind:src="'http://cloud.tidicloud.com'+item.presentImg">{{item.presentName}}</span></span>
	...
	data(){
		...
		methods:{
			...
			star(){
				var self=this;
				if(!self.isNew){
					var url='http://cloud.tidicloud.com/cloud_code/GET/vendorIntegral/getVendorSingleIntegralExchange.do';
					var type='get';
					var data={
			...
			init(currentPage){
				...
				self.rule=true;
				self.gift=false;
				var url='http://cloud.tidicloud.com/cloud_code/GET/vendorIntegral/vendorIntegralRuleList.do';
				var type='get';
				var data={
					vendorId:self.datas.vendorId,
			...
			getGift(currentPage){
				...
				self.rule=false;
				self.gift=true;
				var url='http://cloud.tidicloud.com/cloud_code/GET/product/present.do';
				var type='get';
				var data={
					vendorId:self.datas.vendorId,
					currentPage:currentPage,
			...
			complete(){
				...
				if(!self.isNew){
					var url='http://cloud.tidicloud.com/cloud_code/GET/vendorIntegral/updateExchangesInfo.do';
					var type='post';
					var data={
						ruleId:self.ruleId,
						vendorId:self.datas.vendorId,
				...
				var url='http://cloud.tidicloud.com/cloud_code/GET/vendorIntegral/addExchangesInfo.do';
				var type='post';
				var data={
					ruleId:self.ruleId,
					vendorId:self.datas.vendorId,
